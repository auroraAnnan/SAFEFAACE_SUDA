...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------------------------------------------#
    #   mode用于指定测试的模式：
    #   predict表示单张图片预测，如果想对预测过程进行修改，如保存图片，截取对象等，可以先看下方详细的注释
    #   video表示视频检测，可调用摄像头或者视频进行检测，详情查看下方注释。
    #   fps表示测试fps，使用的图片是img里面的street.jpg，详情查看下方注释。
    #   dir_predict表示遍历文件夹进行检测并保存。默认遍历img文件夹，保存img_out文件夹，详情查看下方注释。
    # ---------------------------------------------------------------------------------------------#
    mode = "video"
    # ---------------------------------------------------------------------------------------------#
    ...
    #   video_path、video_save_path和video_fps仅在mode=video时有效
    ...
    if mode == "predict":
        ...
        while True:
            ...
            if image is None:
                ...
                print("预测时间：", time_end - time_start, "秒")
        ...
        if video_path == 0:
            kill_event = Event()
            # capture = CameraThread(kill_event, video_save_path=video_save_path,
            #                        video_fps=video_fps)
